...
CREATE TABLE CHANGE_TRACKING (
   ...
   TYPE                VARCHAR2(20 CHAR) NOT NULL, -- e.g., INSERT, UPDATE, DELETE
   DATA_CONTENT_TYPE   VARCHAR2(100 CHAR),         -- e.g., application/json
   DATA                CLOB CONSTRAINT CHK_JSON_DATA CHECK (DATA IS JSON),
   DATA_BASE64         CLOB,
   OFFSET_DATE_TIME    TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL
);

CREATE UNIQUE INDEX IDX_CHANGE_TRACKING_OFFSET ON CHANGE_TRACKING(OFFSET_ID);
```
...
```json
{
   ...
   "config": {
      ...
      "table.whitelist": "SYSTEM.CHANGE_TRACKING",
...
```
### Deploy oracle-source-connector.json
```bash
...
### Check status oracle-source-connector.json
```bash
...
### Delete oracle-source-connector.json
```bash
